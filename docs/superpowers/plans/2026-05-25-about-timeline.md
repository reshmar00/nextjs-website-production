...
**Goal:** Replace the static infographic image in `pages/about.tsx` with a scrollable animated zigzag timeline component showing 9 career history entries.

**Architecture:** A `Timeline` component renders a static array of `TimelineEntry` data. Each entry is a `TimelineItem` that uses Framer Motion's `whileInView` for scroll-triggered fade+float animation. Cards are split into a glass gradient header (role + date) and a white body (description). `about.tsx` is reduced to a thin wrapper that mounts `<Timeline />`.

**Tech Stack:** Next.js 14, TypeScript, Chakra UI v2, Framer Motion (all already installed)
...
|------|--------|----------------|
| `pages/components/Timeline.tsx` | **Create** | `TimelineEntry` type, entry data array, `TimelineItem` component, `Timeline` component |
| `pages/about.tsx` | **Modify** | Remove static image; mount `<Timeline />` |

---

### Task 1: Create `pages/components/Timeline.tsx`

**Files:**
- Create: `pages/components/Timeline.tsx`
...
Expected: no errors. If you see errors about `_before`/`_after` object shape, it means a Chakra style prop key is wrong — check the property name (e.g. `pointerEvents` not `pointer-events`).

- [ ] **Step 3: Commit**

```bash
git add pages/components/Timeline.tsx
git commit -m "feat: add glass zigzag Timeline component for About page"
```

---

### Task 2: Wire Timeline into `pages/about.tsx`
...
Note: the outer `<main>` wrapper from the old file is intentionally removed — `_app.tsx` already wraps every page in `<main>`.
...
- Nine cards are visible, zigzagging left/right
- Each card header has the correct gradient color
- Scrolling down causes cards to fade+float in; scrolling back up causes them to fade out again
- All text is uppercase, Raleway font
- White body section below each colored header
- No company names appear anywhere

- [ ] **Step 5: Commit**

```bash
git add pages/about.tsx
git commit -m "feat: replace static infographic with dynamic Timeline on About page"
```
